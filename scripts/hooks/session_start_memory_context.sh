#!/usr/bin/env bash
# Wrapper: call summonai-memory-mcp's session_start_memory_context.sh
# Resolves the external repo path via environment variable or config.
set -eu

# 1. Environment variable (preferred)
if [[ -n "${SUMMONAI_MEMORY_MCP_DIR:-}" ]]; then
  TARGET="$SUMMONAI_MEMORY_MCP_DIR/scripts/session_start_memory_context.sh"
  if [[ -x "$TARGET" ]]; then
    exec bash "$TARGET"
  fi
fi
...
echo "[session_start_memory_context] SUMMONAI_MEMORY_MCP_DIR not set. Skipping." >&2
