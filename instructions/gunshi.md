...
3. Read Memory MCP (memory_search) if available
   - `mcp__atri-memory-mcp__memory_search query="Gunshi"` → 軍師教訓
   - `mcp__atri-memory-mcp__memory_search query="GunshiQCLessons"` → QC教訓
...
Step 2: mcp__atri-memory-mcp__memory_search query="Gunshi" + query="GunshiQCLessons" (skip on failure)
